...
library(dplyr)    # imported for tiblle, pipe, etc.
```
---
...
with(pikachu, tibble(
    name = "Pikachu",
    weight = weight/10, # API in hectograls, we want kilograms
...
# Mais si je m'interesse aussi à Bulbizarre, Salamèche et Carapuce ?
...
Mais :
* verbeux
* crée des variables dans l'environement global
...
* non immédiatement parralelisable
...
* en théorie, moins besoin de mettre les mains dans le camboui
...
* moins besoin de mettre les mains dans le camboui
* facilement parralelisable :
...
**Tout** le calcul est perdu à cause d'une erreure sur **une seule** ittération 
(T__T)
...
Attention : les erreures sont devenues silencieuses !
```{r error=TRUE}
...
- éxécuter sur un sous ensemble : `map_at(x, at, fun)`, `map_if(x, cond, fun)`
...
```{r error = TRUE}
possibly_get_pokemon_bmi(c("pikachu", "bulbasaur", "charmander", "squirtle", "magicarpe"))

get_pokemon_bmi_v <- function(poke_names, sleep = 0) {
    map_dfr(poke_names, possibly_get_pokemon_bmi, sleep = sleep)
}

get_pokemon_bmi_v(c("pikachu", "bulbasaur", "charmander", "squirtle", "magicarpe"))
